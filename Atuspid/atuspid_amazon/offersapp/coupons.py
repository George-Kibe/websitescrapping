...
def getdeals(soup):
    offer_or_coupon_items = soup.find_all(
        'div', {'class': 'a-section a-spacing-none a-text-left coupon'})
    for item in offer_or_coupon_items:
        title = item.find(
            'div', {'class': 'a-section a-spacing-small coupon-description'}).text.strip()
        ...
        image_url = item.find(
            'img', {'class': 'a-dynamic-image coupon-image'})['src']

        try:
            coupon = item.find('span', {
                               'class': 'a-size-medium a-color-success a-text-bold'}).text.replace('Save', '').strip()
