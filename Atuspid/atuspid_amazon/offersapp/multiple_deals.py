...
def getdeals(soup):
    ...
    for item in offer_or_coupon_items:
        ...
        try:
            saleprice = float(item.find_all('span', {
                              'class': 'a-offscreen'})[0].text.replace('$', '').replace(',', '').strip())
            oldprice = float(item.find_all('span', {
                             'class': 'a-offscreen'})[1].text.replace('$', '').replace(',', '').strip())
        except:
            saleprice = "Missing"
            oldprice = "Missing"
        try:
...
def getnextpage(soup):
    ...
    if not pages.find('span', {'class': 's-pagination-item s-pagination-next s-pagination-disabled'}):  # last page
        url = 'https://www.amazon.com' + \
            str(pages.find('a', {
                'class': 's-pagination-item s-pagination-next s-pagination-button s-pagination-separator'})['href'])
        return url
    else:
        return
