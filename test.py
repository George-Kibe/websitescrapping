...
def getdeals(soup):
    daily_deals = soup.find_all('div', {'class': 'DealGridItem-module__dealItemContent_1vFddcq1F8pUxM8dd9FW32'})
    for item in daily_deals:
        title = item.find('div', {'class': 'DealContent-module__truncate_sWbxETx42ZPStTc9jwySW'}).text.strip()
        refined_link = item.find('a', {'class': 'a-link-normal a-color-base a-text-normal'})['href']
        image_url= item.find('img')['src']
        try:
            saleprice = float(item.find('span', {'class': 'a-price-whole'}).text.strip())
            offer_details =item.find('span', {'class':'a-size-small a-color-secondary'}).text.strip()
        except:
            saleprice="Missing"
